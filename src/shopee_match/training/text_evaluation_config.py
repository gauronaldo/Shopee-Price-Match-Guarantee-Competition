"""Strict frozen-checkpoint configuration for one-time Phase 4 test evaluation."""
...
from shopee_match.training.text_config import (
    ...
    _relative_path,
    _typed,
    load_text_experiment_config,
)


@dataclass(frozen=True, slots=True)
class FrozenTextCheckpointConfig:
    path: Path
    sha256: str
...
@dataclass(frozen=True, slots=True)
class FrozenTextTestConfig:
    config_version: str
    seed: int
    device: str
    num_workers: int
    batch_size: int
    checkpoint: FrozenTextCheckpointConfig
    training_experiment: TextExperimentConfig
    recall_at: tuple[int, ...]
...
def load_frozen_text_test_config(path: Path) -> FrozenTextTestConfig:
    """Load and verify immutable text artifacts plus a validation-frozen protocol."""
    root = _read_yaml(path, "frozen text evaluation config")
    _only_keys(
        root,
        {"config_version", "seed", "runtime", "frozen", "evaluation", "artifacts"},
        "config",
    )
    if root["config_version"] != "phase4.frozen_text_test.v1":
        raise ConfigurationError("Unsupported frozen text evaluation config_version")
    ...
    checkpoint_sha256 = _sha256_digest(frozen["checkpoint_sha256"], "frozen.checkpoint_sha256")
    training_config_sha256 = _sha256_digest(
        frozen["training_config_sha256"], "frozen.training_config_sha256"
    )
    training_metrics_sha256 = _sha256_digest(
    ...
    training_experiment = load_text_experiment_config(training_config_path)
    ...
    if evaluation["split"] != "test":
        raise ConfigurationError("Frozen text evaluation may target only the test split")
    if (
    ...
    if (
        recall_at != training_experiment.evaluation.recall_at
        or average_precision_at != training_experiment.evaluation.average_precision_at
